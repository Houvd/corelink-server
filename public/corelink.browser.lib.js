...
(function () {
  ...
  */
   ...
   *@param {String} options Options object,
                            if generic is called only the complete Json needs to be send
                            and it will execute
  ...
  async function generic(options) {
    return (new Promise(async (resolve, reject) => {
      ...
      if (!(('json' in workOptions))) reject(new Error('No JSON found.'))

      let request
      if (workOptions.json) {
        request = JSON.parse(workOptions.json)
        if (!('function' in request)) reject(new Error('No function name'))
        if (workOptions.token) request.token = workOptions.token
        else { request.token = token }
