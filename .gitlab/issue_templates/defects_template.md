# _Issue Title_

## _Overview_

>this captures the essence of what is needed. It usually describes the use case at hand e.g. for a bug request, "error while uploading a file" could be a possible statement with more description about how, when and where did the bug occur.

## _Problem_

>this captures what is going in detail. This is usually an extension of the Overview itself in bug tickets.

### _Where_

### _Branch_

### _Steps To Reproduce_

### _Components with the same problem or effected by this_

### _Additional Information_

>this usually captures if there are debug logs, traces, stack dumps, previous analysis done on the problem which might be useful to the developer.

## _Requirements_

### _Current behavior_

> this captures what is happening currently.

### _Expected behavior_

>this captures what is needed. this needs to be as descriptive as possible.

## _Acceptance Criteria_

>defines what is considered an acceptable fulfillment of the ticket. Again, this definition needs to be objective and solidly outlined.

## _Testing Requirements_

/label ~issue_priority_unknown ~issue_type_defect ~tech_type_generic
/target_branch master
